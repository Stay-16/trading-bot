...
- The Telegram bot will show an `Open Mini App` button automatically when `WEBAPP_URL` is configured
...
4. Promote the current backend into a shared platform for Telegram, Web, Desktop, and Mobile
5. Follow `MULTI_APP_ROADMAP.md` to keep all clients on one backend contract
