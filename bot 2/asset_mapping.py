...
def is_preferred_live_entry(
    entry: Dict[str, Any],
    pairs: Dict[str, Dict[str, Any]],
) -> bool:
    pairdict = entry.get("pairdict", {}) or {}
    market_type = entry.get("market_type") or pairdict.get("type")
    normalized_symbol = normalize_quotex_symbol(entry.get("quotex_symbol") or pairdict.get("quotex_symbol") or "")
    known_symbols = {
        normalize_quotex_symbol(pair_info.get("quotex_symbol", ""))
        for pair_info in pairs.values()
        if pair_info.get("quotex_symbol")
    }
    ...
    if market_type == "regular":
        base_symbol = normalized_symbol[:-4] if normalized_symbol.endswith("-OTC") else normalized_symbol
        return len(base_symbol) == 6 and base_symbol.isalpha()

    return market_type == "crypto"
