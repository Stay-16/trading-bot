...
async def on_button(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # ── تحليل فوري / الإشارة التالية ──────────────────
        ...
        try:
            ...
            pass
        # Gemini verification for next signal
        gemini_key = os.getenv("GEMINI_API_KEY", "")
        if gemini_key and len(candles) >= 10:
            try:
                await q.message.reply_text("🔍 *Gemini AI Verifying...* ⏳", parse_mode=ParseMode.MARKDOWN)
                chart_png = generate_chart_image(candles, B.symbol, title=f"{B.symbol} - Next Signal")
                report = _build_technical_report(sig, candles, B.symbol)
                decision, gemini_text = await analyze_with_gemini(chart_png, report)
                if decision == "CANCEL":
                    await q.message.reply_text(
                        f"🛑 *تم إلغاء الإشارة* — Gemini رفضها\n📋 {gemini_text[:600]}",
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=ui_layer.build_signal_keyboard(is_running=True),
                    )
                    return
                elif decision == "CONFIRMED":
                    sig.reasons.append("🤖 Gemini AI: CONFIRMED (بصري + رقمي)")
                    sig.confidence = min(95, sig.confidence * 1.05)
            except Exception as e:
                log.warning("Gemini verification skipped: %s", e)
    ...
    # ── تأكيد وبدء التحليل اليدوي ────────────────────
        ...
        # 3. التحقق عبر Gemini (صورة شارت + تحليل بصري)
        gemini_key = os.getenv("GEMINI_API_KEY", "")
        if gemini_key and len(candles) >= 10:
            try:
                await q.message.reply_text("🔍 *Gemini AI Verifying...* ⏳", parse_mode=ParseMode.MARKDOWN)
                chart_png = generate_chart_image(candles, B.symbol, title=f"{B.symbol} - Manual Signal")
                report = _build_technical_report(sig, candles, B.symbol)
                decision, gemini_text = await analyze_with_gemini(chart_png, report)
                if decision == "CANCEL":
                    await q.message.reply_text(
            ...
            log.info("Gemini not configured (GEMINI_API_KEY missing), skipping visual verification")
    ...
    # ── تسجيل النتائج ──────────────────────────────────
        ...
        B.alert_builder.record_result(B.symbol, "LOSS", 0, False, loss)
        if B.manager:
            B.manager.stats.losses             += 1
            B.manager.stats.consecutive_losses += 1
        B.risk_manager.record_trade_result(profit=loss)
        await _learn("loss", loss)
        if B.consecutive_losses >= 3:
            critical = (
                f"🚨 *Critical Alert — Circuit Breaker*\n"
                f"{B.consecutive_losses} losses in a row | Balance: `${B.balance:.2f}`\n"
                f"Bot will reduce position size until a win occurs."
            )
            try:
                await B.send(critical)
            except Exception:
                pass
        if loss < 0 and B.balance > 0:
            state = B.risk_manager._get_state()
            daily_loss_pct = abs(min(state.daily_pnl, 0)) / B.balance * 100
            limit_pct = B.risk_manager.daily_loss_limit_pct * 100
            if daily_loss_pct >= limit_pct * 0.8:
                critical = (
                    f"⚠️ *Daily Loss Warning*\n"
                    f"Losses: {daily_loss_pct:.1f}% / {limit_pct:.0f}% limit\n"
                    f"Bot will stop trading if limit is reached."
                )
                try:
                    await B.send(critical)
                except Exception:
        ...
        await q.message.reply_text("⏭ تم تجاهل الصفقة.")
